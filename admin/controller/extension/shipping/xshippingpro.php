...
class ControllerExtensionShippingXshippingpro extends Controller {
    ...
    public function load_method() {
       ...
       $data['method_data'] = array($this->model_extension_shipping_xshippingpro->getDataByTabId($tab_id));
       $options = $this->getFormBuilderOptions($data);
       $this->ocm->form->setLangs($ext_lang)->setOptions($options);
    ...
    public function save_general(){
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
             ...
             $this->model_extension_shipping_xshippingpro->saveSort($sorted);
             $json['success'] = true;
        } else{
             $json['error']=$this->language->get('error_permission');
        }
    ...
    private function getFormData($data, $new_tab = false) {
        ...
        $range_types = array('quantity', 'weight', 'volume', 'dimensional', 'volumetric', 'sub', 'total', 'sub_coupon', 'total_coupon', 'grand_shipping', 'grand', 'no_product', 'no_category', 'no_manufacturer', 'no_location', 'distance', 'equation');
    ...
    private function getSubOptionForm($data, $sub_options = array(), $new = false) {
         ...
         foreach ($sub_options as $sub_option) {
             ...
             $return .= '</tr>';
             $return .= '</table>';
             $return .= '<div class="sub-option-action"><button class="btn btn-danger btn-sm delete-sub-option"><i class="fa fas fa-times"></i></button><button  class="btn btn-default btn-light btn-sm drag-sub-option"><i class="fa fas fa-arrows-alt drag-pivot"></i></button></div></div>';
             $counter++;
        }
    ...
    private function getRanges($method_data, $data, $no_of_tab) {
        ...
        $return .= '   <a href="'.$data['export'].'&no='.$no_of_tab.'" class="btn btn-info export-btn btn-sm range-btn" role="button">'.$data['text_export'].'</a>';
