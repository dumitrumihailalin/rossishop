...
class ModelExtensionShippingXshippingpro extends Model {
    ...
    function getQuote($address) {
       ...
       if ($xshippingpro_group != 'no_group' && $method_level_group) {
        ...
        if ($xshippingpro_debug) {
            ...
            array_unshift($debugging, array('name' => 'Processed Done in ' . $_req_time . ' seconds', 'filter' => array(),'index' => ''));
    ...
    private function iterateEquation($equation, $type, $rates, $method_specific_data, $_cart_data, $quote_data, $percent_of, $cost = 0, $modifier_amount = 0) {
        ...
        foreach ($method_specific_data['products'] as $product) {
            ...
            $target_value = $this->getEquationValue($equation, $_cart_data, $method_specific_data, $quote_data, $percent_of, $_prev_cost, $modifier_amount);
    ...
    private function getProductProfile($cart_products, $xmeta) {
        ...
        foreach($cart_products as &$product) {
            ...
            $total_with_tax = $this->tax->calculate($product['price'], $tax_class_id) * $product['quantity'];
    ...
    private function _resetEmptyRule($data) {
        ...
        if (!isset($data['disable_other'])) $data['disable_other'] = '';
        ...
        $proversion = array('date_start', 'time_start', 'max_length', 'max_width', 'max_height', 'weight_start', 'quantity_start', 'equation', 'rate_final', 'rate_percent', 'cart_adjust', 'rate_min', 'rate_max', 'rate_add', 'additional', 'error', 'group', 'logo', 'order_total_start');
    ...
    private function _findRawRate($data) {
        ...
        if ($data['rate_type'] == 'flat') {
           ...
           foreach($data['ranges'] as $range) {
               ...
               $product_id = isset($range['product_id']) ? (int)$range['product_id'] : 0; // latency compatiblity 
    ...
    private function _getApplicableProducts($rules, $cart_data) {
        ...
        // no of unique prouct with diff options
        $no_product = 0;
        if ($_applicable['product']) {
            foreach ($cart_data['products'] as $product) {
                if (in_array($product['product_id'], $_applicable['product'])) {
                    $no_product++;
                }
            }
        }
        $_applicable['no_product'] = $no_product;
    ...
    public function getScript() {
        ...
        if ($shipping_xshippingpro && (($this->ocm->isCheckoutPage() && !$this->ocm->isCartPage()) || ($product_id && $estimator_on_store && isset($xshippingpro_estimator['status'])))) {
            ...
            $html .= '<script src="catalog/view/javascript/xshippingpro.min.js?v=1.0.4" defer type="text/javascript"></script>';
        }
        return $html;
    }
            ...
            if ($shortcodes = $this->ocm->parseShortcode($text)) {
                foreach ($shortcodes as $shortcode) {
                    ...
                    if (method_exists($this, $render) && $_return = $this->{$render}($shortcode)) {
                        ...
                        $return[$shortcode['name']] = array($tab_id => $_return['output']);
    ...
    private function _xformRender($shortcode) {
        ...
        if ($this->getXformStatus()) {
            ...
            $output = $xform->renderForm($formId, array(), false);
            $output .= '<style>.xform-submit{display:none !important;}</style>';
            $output .= '<script type="text/javascript">window.xform.warmUp();</script>';
            $return = array(
                'output' => $output,
                'apply'  => false
            );
    ...
    private function _dateRender($shortcode) {
        ...
        $time = time();
        $w = date('w', $time);
        $days = (int)$shortcode['text'];
        // if today is NOT in offday list, decrease days by 1
        if (!in_array($w, $param['offday']) && $param['same_day']) {
        ...
        return array(
            'output' => date($param['format'], $time),
            'apply'  => true
        );
    }
    private function getXFormAsset($xform_str) {
        $return = '';
        if ($this->getXformStatus() && strpos($xform_str, 'bootstrap-xform') !== false) {
            $xform = new \Xform($this->registry);
            $fake_fields = array();
    ...
    public function getXformStatus() {
        if ($this->ocm->isCacheAvail('xform_avail')) {
            return $this->ocm->getCache('xform_avail');
        }
        $xform_mod = $this->db->query("SELECT * FROM " . DB_PREFIX . "extension WHERE `code` = 'xform'")->row;
        $this->ocm->setCache('xform_avail', (bool)$xform_mod);
        return $this->ocm->getCache('xform_avail');
    }
    /* HOOK METHOD HERE */
